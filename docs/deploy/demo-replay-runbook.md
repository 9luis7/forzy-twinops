...
6. Com a reprodução em andamento, abrir **Copiloto** e perguntar sobre S2. O backend captura uma revisão consistente ao receber cada pergunta. A reprodução continua; a resposta identifica a linha e o horário analisados. Perguntas seguintes, inclusive “e S1?”, capturam outro recorte e usam a conversa apenas para interpretar a referência.
...
`DEMO_RAG_EQUIPMENT_MODEL` seleciona opcionalmente a identidade do equipamento usada na busca e geração documental da demonstração e do copiloto histórico. Para o corpus WEG reconstruído, definir `DEMO_RAG_EQUIPMENT_MODEL=W22`, correspondente à família identificada no manual. Preservar `TWINOPS_RAG_EQUIPMENT_MODEL=W22 13887610` para o live. Sem o override, ambos herdam o modelo live; a identidade precisa corresponder exatamente à do corpus publicado. Esse vínculo documental não valida a posição dos sensores nem amplia a cobertura do manual à bomba.

Com uma conexão dedicada distinta em `DEMO_DATABASE_URL`, o startup não inicializa o Neon, mesmo com `DEMO_ENABLED=false`. Uma rota live tenta inicializá-lo sob demanda; falha retorna `503 live_unavailable`, `Cache-Control: no-store` e `Retry-After: 30`. Depois desse intervalo uma nova solicitação pode tentar recuperar o live. A leitura histórica e, quando configurado, o copiloto histórico permanecem independentes da disponibilidade do Neon e da habilitação do replay. A busca dedicada não substitui um corpus ausente pelo corpus live.
...
Para desligar somente a demonstração, configurar `DEMO_ENABLED=false` e publicar um novo deployment. Alterar a variável sozinha não modifica deployments imutáveis já publicados. Com o banco dedicado configurado, o histórico e o copiloto histórico continuam disponíveis; esse flag não reverte as alterações do SaaS nem restaura a dependência original do Neon. Para reverter o código, usar o rollback de deployment, que restaura o comportamento da versão escolhida. Nenhuma dessas ações recupera uma conexão bloqueada por cota. As tabelas dedicadas podem permanecer e o rollback não precisa apagar dados ou alterar tabelas live. A correção da coleta semanal e integrações com ordens de serviço ficam fora desta entrega.
