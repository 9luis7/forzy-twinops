# Unified Twin Acceptance Ledger

- Schema: `unified-twin-acceptance-v1`
- Frozen spec SHA-256: `20629d860f19a212bc2b941d6270e2316c747a0b85c473f0b51bbee6106e3a44`

## Plans

| Plan | Status | Verified code commit | Review verdict |
| --- | --- | --- | --- |
| A | pending | pending | pending |
| B | pending | pending | pending |
| C | pending | pending | pending |
| D | pending | pending | pending |
| E | in_progress | 82cb338a0dbd18bce3dd625909c5dc0250ccdeb2 | 0/0/0 |

## Acceptance criteria

| Criterion | Owner | Gate | Allowed evidence | Status | Evidence |
| --- | --- | --- | --- | --- | --- |
| AC-01 | C | local | automated | pending | pending |
| AC-02 | C | local | automated | pending | pending |
| AC-03 | C | local | automated | pending | pending |
| AC-04 | C | local | automated | pending | pending |
| AC-05 | C | local | automated | pending | pending |
| AC-06 | A | database | database | pending | pending |
| AC-07 | C | local | automated | pending | pending |
| AC-08 | D | local | automated | pending | pending |
| AC-09 | D | local | automated | pending | pending |
| AC-10 | D | local | automated | pending | pending |
| AC-11 | C | local | automated | pending | pending |
| AC-12 | E | preview | preview | pending | pending |
| AC-13 | B | local | automated | pending | pending |
| AC-14 | B | local | automated | pending | pending |
| AC-15 | E | local | automated | pending | pending |
| AC-16 | C | local | automated | pending | pending |
| AC-17 | C | local | automated | pending | pending |
| AC-18 | E | human | human | pending | pending |
| AC-19 | C | local | automated | pending | pending |
| AC-20 | D | local | automated | pending | pending |
| AC-21 | C | local | automated | pending | pending |
| AC-22 | C | local | automated | pending | pending |
| AC-23 | D | local | automated | pending | pending |
| AC-24 | E | preview | preview | pending | pending |
| AC-25 | C | local | automated | pending | pending |
| AC-26 | C | local | automated | pending | pending |
| AC-27 | C | local | automated | pending | pending |
| AC-28 | E | human | human | pending | pending |

## Findings

| ID | Plan | Severity | Status | Introduced SHA | Resolved SHA |
| --- | --- | --- | --- | --- | --- |
| E1-C-001 | E | critical | fixed | 3720634d74dc3ec0364674a722a6bcdfd6a6657d | a57ff73ef5ad53307d8b852a7072333ded570f56 |
| E1-I-001 | E | important | fixed | 3720634d74dc3ec0364674a722a6bcdfd6a6657d | a57ff73ef5ad53307d8b852a7072333ded570f56 |
| E1-I-002 | E | important | fixed | 3720634d74dc3ec0364674a722a6bcdfd6a6657d | a57ff73ef5ad53307d8b852a7072333ded570f56 |
| E1-I-003 | E | important | fixed | 3720634d74dc3ec0364674a722a6bcdfd6a6657d | a57ff73ef5ad53307d8b852a7072333ded570f56 |
| E1-I-004 | E | important | fixed | 3720634d74dc3ec0364674a722a6bcdfd6a6657d | a57ff73ef5ad53307d8b852a7072333ded570f56 |
| E1-I-005 | E | important | fixed | 3720634d74dc3ec0364674a722a6bcdfd6a6657d | a57ff73ef5ad53307d8b852a7072333ded570f56 |
| E1-I-006 | E | important | fixed | 3720634d74dc3ec0364674a722a6bcdfd6a6657d | a57ff73ef5ad53307d8b852a7072333ded570f56 |
| E1-I-007 | E | important | fixed | 3720634d74dc3ec0364674a722a6bcdfd6a6657d | a57ff73ef5ad53307d8b852a7072333ded570f56 |
