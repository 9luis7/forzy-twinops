...
const EVENT_STATES = { pending: "Na fila", processing: "Gerando análise", ready: "Análise disponível", degraded: "Resposta limitada" };
...
function EventStatus({ event, recommendationPending }) {
  ...
  return <span className="small muted" role="status" title={awaiting ? "Consulta enviada; aguardando resposta do serviço." : undefined}>
    {awaiting ? "Gerando análise" : EVENT_STATES[event.status]}
  </span>;
}
...
export default function DemoDashboard({ dataSource }) {
  ...
  const canQuery = Boolean(replay?.sourceRow)
    && (!busy || state.pendingOperation === "advance") && !state.queuedAction && !state.manualPending && !error;
  const sendQuestion = (value) => {
    const current = controller.getSnapshot();
    if (!value.trim() || !current.context?.replay.sourceRow || current.manualPending || current.queuedAction || current.error
      || (current.busy && current.pendingOperation !== "advance")) return;
    setEventSelection(null);
    setQuestion(value);
  ...
  const openAnalysis = () => {
    if (!canQuery) return;
    setEventSelection(null);
    setQuestion(demoSuggestions(controller.getSnapshot().context)[0].question);
    setCopilotOpen(true);
  };
  ...
  return <main className={`demo-shell${copilotOpen ? " has-copilot-open" : ""}`} onKeyDown={keyboard} tabIndex={0} aria-label="Painel de replay, Espaço para reproduzir ou pausar e seta direita para um par">
    ...
    <section className="panel demo-controls" aria-label="Controles do replay">
      ...
      {context && <>
        <div className="demo-playback"><button disabled={controlBusy || replay.state === "completed" || Boolean(error)} onClick={toggle}>{state.queuedAction === "pause" ? "Pausa solicitada…" : running ? "Ⅱ Pausar" : "▶ Continuar"}</button>
          ...
          {!attentionSensors.length && <button className="secondary-button" disabled={!canQuery} onClick={openAnalysis}>Analisar este instante</button>}
    ...
    <section className="demo-flow panel" aria-label="Como a demonstração funciona">
      <ol><li><strong>1. Dados chegam</strong><span>O histórico avança em pares de S1 e S2.</span></li><li><strong>2. O sistema avalia</strong><span>ML e scores atualizam gráficos e 3D.</span></li><li><strong>3. Um evento é registrado</strong><span>Quando a atenção persiste ou o estado muda.</span></li><li><strong>4. O copiloto analisa</strong><span>Por evento ou por uma pergunta sua.</span></li></ol>
    ...
    {!context ? <section className="demo-welcome panel"><h2>Escolha um roteiro e prepare a demonstração.</h2><p>Depois, use <strong>Continuar</strong> para receber os dados em sequência ou <strong>Avançar 1 par</strong> para explorar aos poucos. Use <strong>Analisar este instante</strong> para consultar o copiloto enquanto o replay continua.</p><p className="small muted">O roteiro guiado mostra 300 pares. O roteiro livre permite percorrer o histórico completo.</p></section> : <>
      {attentionSensors.length > 0 && <section className="panel demo-attention" aria-label="Atenção no conjunto" data-status={context.status}>
        ...
        <button onClick={openAnalysis} disabled={!canQuery}>Analisar este instante</button>
      </section>}
    ...
    <CopilotDock open={copilotOpen} onOpenChange={setCopilotOpen} notificationCount={readyCount} contextLabel={replay?.sourceTime ? `Demonstração · medição histórica de ${fullClock(replay.sourceTime)}` : "Demonstração · aguardando uma leitura do histórico"}>
      <section className="demo-copilot demo-assistant" aria-label="Copiloto da demonstração">
        {selectedEvent && <article className="demo-event-answer" ref={eventAnswerRef} tabIndex={-1} aria-label="Orientação do evento selecionado">
          <p className="eyebrow">Análise automática</p><h3>{EVENT_LABELS[selectedEvent.kind]}</h3>
        ...
        <p className="eyebrow">Dados + ML + documentação</p><h2>Pergunte sobre este instante</h2><p className="muted small">Cada pergunta consulta novamente os dados. A resposta identifica a última leitura incluída na análise, enquanto o replay continua. O manual cobre o motor; os sinais e scores da bomba também podem ser explicados.</p>
